...
- 카메라가 보는 것을 로그로 인코딩 : 이미지 노출(정지 상태에서 측정)과 저장된 이미지 사이의 연관성이 더 넓은 범위에서 완전히 일정하다는 것을 의미

- 사람의 눈이나 비디오 화면을 위한 단순 캡쳐보다는 최대한 많은 데이터를 저장하기 위해서 표준영상curve보다 센서의 정보를 더 많이 활용
  (= 후반작업때 더 많은 컬러데이터를 제공해준다.)

</code>
</pre>

- Linear Curve & Log Curve
...
- Linear Curve : 일정하고 변화가 없는 밝기를 보여줌

- Log Curve : 그림자를 유지하기 위해 어두운 영역을 위로 올리고, 하이라이트를 유지하기위해 밝은영역은 아래로 내린다.
              색상곡선의 각각의 측면에서 더 많은 데이터 보존이 가능하다.
...
- 우리는 이미지가 Linear로 처리되기를 원하지만, sRGB로 저장이 되는 문제가 발생
  이러한 문제는 Linear와의 호환문제를 야기시킨다. 해결 방법으로 8bit에서 16/32bit로 범위를 넓히는 것
  
- 포토샵은 8/16bit에서는 잘 작동하지만 32bit linear 이미지를 8/16bit sRGB 공간에서 보는것은 이미지의 데이터 손실이 발생함.
