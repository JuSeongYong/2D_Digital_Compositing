...
파라메트릭 곡선과 표면에 대한 피에르 베지에의 초기 작업이 공개되었고, IBM은 레이 트레이싱의 전구체인 숨겨진 표면과 그림자 알고리즘을 개발했다.
더그 잉글바트는 컴퓨터 마우스를 발명했다.

- 1970년대

렌더링(shading)은 유타 대학의 구라우와 퐁에 의해 발견되었다.
3D 그래픽용 키프레임 기반 애니메이션을 처음으로 시연했다.
제록스 PARC는 "Paint" 프로그램을 개발했다.
에드 캣멀은 z-buffer 알고리즘을 도입했고 턴 위트는 recursive ray tracing을 개발했고, 이는 후에 photorealism의 표준이 되었다.
Pong, Pac-Man과 같은 아케이드 게임은 매우 유명해졌다.

- 1980년대

1980년대 중반에 가까운 시기에 짐 블린은 블로비 모델과 텍스처 맵핑 개념을 도입했다.
...
데이터 글로브에서 시작된 모션 캡쳐는 애니메이션 시퀀스를 생성하는 일차적인 방법이 되었다.
