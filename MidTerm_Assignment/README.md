...
컴퓨터의 발전으로 매트페인팅은 두가지의 기술적인 변화가 일어났다.

1. 유화와 붓을 사용한 전통적인 회화기법에서 포토샵과 그래픽 태블릿을 이용한 디지털 드로잉으로 변화한 것이다.

- 매트페인터가 그림을 그리면서 여러 사진을 참조할 수 있음.
- 그림의 수정과 제작이 훨씬 빨라짐; 이전에 그렸던 그림이 기초를 제공해줌.

2. 3D Projection

- 3D환경을 사용해 입체적으로 그림을 투영시킬 수 있다.
- 훨씬 복잡하고 사실적인 표현이 가능해짐.
- 평평한 배경에서 입체적인 환경을 제작할 수 있다.
...
컴퓨터에 의한 영상처리. 미국 매사추세츠공대(MIT) 박사과정을 이수하고 있던 I. 서덜랜드가 1963년에 발표한 「스케치패드 시스템」에 과한 논문에서 시작되었다.
실제로 볼 수 없는 화상을 표시할 수 있다는 이점이 있고 컴퓨터가 출력하는 정보를 시각화하기 때문에 응용범위가 넓다.
최근에는 CG로 약칭되는 경우도 많다. 현재 의약품개발, 지도, 측량, 디자인분야 등 다양한 시스템을 개발, 실용화되고 있다.
...
최초의 Interactive Computer Graphic을 제공해줬다. 컴퓨터 그래픽의 시작이라고 할 수 있다.
...
렌더링(shading)은 유타 대학의 Gouraud와 Phong에 의해 발견되었다.

3D 그래픽용 키프레임 기반 애니메이션을 처음으로 시연했다.
...
Computer Aided Design 강좌를 여러 대학에서 가르치기 시작했다.

데이터 구조로 BSP(Binary Space Partitioning : 이진 공간 분할법) 트리가 도입되었지만, 그것들이 얼마나 유용해질지 깨닫는 사람은 많지 않았다.

몇몇 예술가들이 컴퓨터 그래픽의 프랙탈을 탐구하기 시작했다.
...
충돌·중력·마찰·인과관계로 애니메이션을 가능케 한 역학 시스템이 도입됐다.

최초의 CAD 대학이 개설되었다.

3dfx나 엔비디아와 같은 PC 그래픽 카드가 도입되었다.
...
시각 효과(Visual Effects - 보통 Visual FX 또는 VFX로 칭합니다.)는 실제 촬영이 불가능한 영상이나 기타 움직이는 물체를 생성하고 변경하거나 또는 더욱 훌륭한 이미지를 제작하는 데 사용되는 용어입니다.
다시 말해, VFX 기술은 1차로 영상의 실사 촬영이나 기본적인 제작이 완료되고 난 뒤, 후반의 작업에서 이루어집니다.
시각 효과는 다양한 방식으로 기록된 이미지 합성 등의 기법을 통해 실제로 촬영한 영상에 합성을 할 수가 있습니다.

시각적 효과는 실제와 같이 보이지만, 위험하거나 비용이 많이 들거나, 필름에 담기가 불가능할 수 있는 환경을 만들기 위해 실사 영상과 생성된 이미지를 통합하는 것을 통틀어 말합니다. 컴퓨터 생성 이미지를 사용한 시각 효과는 저렴한 애니메이션 및 컴포지팅 소프트웨어의 도입으로 최근 독립 영화 제작자가 이용할 수 있게 되었습니다.

- SFX - Special Effects

특수 효과 (Special Effects - 보통 SFX, SPFX 또는 간단히 FX로 칭합니다.)는 스토리, 가상 세계에서 상상되는 이벤트를 시뮬레이션하기 위해 영화, TV, 극장, 비디오 게임 및 시뮬레이터 산업에서 사용되는 환상 또는 시각적 효과입니다.

특수 효과는 전통적으로 광학 효과와 기계적 효과의 범주로 나뉩니다. 디지털 영화 제작의 등장으로 특수 효과와 시각 효과 사이의 구별이 커졌고, 후자는 디지털 후반 작업을 참조하고 "특수 효과"는 기계 및 광학 효과를 나타냅니다.

보통 촬영을 하기 위해 시설물이나 환경, 도구 등을 직접 제작하여 촬영하는 방법입니다. 특수분장, 애니메트로닉스, 미니어처, 폭발, 고속촬영 등의 영상제작기법등이 있습니다.

<pre>
<code>

   - 특수분장 : 사람이 화장이나 특수하게 제작된 가면, 탈, 인형옷등을 입고 카메라 앞에서 실제로 촬영하는 방법

   - 애니메트로닉스 : 인형이나 로봇으로 모형으로 제작을 하여 그 움직임을 입력해서 촬영하는 방법

   - 미니어처 : 실물을 작게 만들어서 촬영하는 방법

   - 폭발 : 화약이나 기타 광원 등을 이용해서 촬영할때 실제 폭발하는 것을 촬영하는 방법

   - 고속촬영 : 슬로우 모션을 보여주기 위해 사용 되는 촬영 방법. 일반적인 영화 필름은 24프레임 이지만 고속촬영에 사용되는 필름은 120프레임.
               주로 극적인 순간이나 아주 자세히 표현 하고자 할때 사용.
   
</code>
</pre>

CG 기술은 처음에는 이러한 SFX를 보조하는 수단으로 사용되기 시작하다가, 기술발달에 따라 그 쓰임새가 넓어지면서 차츰차츰 SFX의 각 영역을 대체하기 시작했다.
나아가 디지털 색보정 등 기존의 SFX 영역을 넘어 촬영의 영역까지 CG의 역할이 확장되면서 사람들은 이를 부르기 위한 새로운 용어가 필요하게 되었고, 그것이 곧 VFX이다.
현재 영화산업 현장에서는 여전히 고전적인 의미의 특수분장, 특수촬영, 스턴트 팀과 함께 VFX팀이 CG를 담당하는 팀의 의미로 별도로 존재하고 있는데 차츰 VFX를 총괄하는 VFX 수퍼바이저가 다른 SFX팀의 결과물까지 총괄하는 책임자로 변모하고 있고, 이러한 경향은 앞으로 더욱 강화되어 VFX 수퍼바이저는 영화의 시각적인 측면을 책임지는 데 있어 기존의 촬영감독과 어깨를 나란히 하게 될지도 모른다.

![Alt](https://github.com/JuSeongYong/2D_Digital_Compositing/blob/master/MidTerm_Assignment/Images/CG%EB%8C%80%EC%B2%B4.jpg)
CG 사용 전후의 특수시각효과의 변화
